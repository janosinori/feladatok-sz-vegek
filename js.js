...
// stinget adjon vissza!
console.log('2.feladat');

const user = {
    firstName:'Nóri',
    lastName:'Kissné Jánosi',
    age:'31'
}

const tag = (texts,...values) => texts.map((text,index) => `<em>${text}</em>${values[index] ? `<strong>${values[index]}</strong>` : ``}`).join('');
const template= tag `A nevem ${user.lastName} ${user.firstName} és ${user.age} éves vagyok.`;
